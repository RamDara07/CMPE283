...
struct capability_info procbased_prim[22] =
{
...
struct capability_info procbased_second[28] =
{
    ...
    { 23, "Sub-page writepermissions for EPT" },
...
{
    uint32_t lo, hi,check_secondary_exists, check_tertiary_exists ;
    ...
    pr_info("Procbased Primary Controls MSR: 0x%llx\n",
        (uint64_t)(lo | (uint64_t)hi << 32));
    report_capability(procbased_prim, 22, lo, hi);
    ...
    if(check_secondary_exists){
        ...
        pr_info("Secondary Procbased controls MSR: 0x%llx\n",
            (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(procbased_second, 28, lo, hi);
    }
    else {
    ...
    check_tertiary_exists = (hi & (1 << 17)) ? 1 : 0;
    if(check_tertiary_exists){
